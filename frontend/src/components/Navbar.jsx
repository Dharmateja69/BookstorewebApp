...
const Navbar = () => {
  ...
  const element = document.documentElement;//code for theme dark ,light;
  useEffect(()=>{
    if(theme==="dark"){
...
const handelscroll=()=>{
  ...
  return (
    <div className={'max-w-screen-2xl  dark:bg-slate-900 dark:text-white container mx-auto md:px-20 px-4 fixed top-0 left-0 right-0 z-50 ${sticky? "sticky-navbar shadow-md bg-base-100 dark:bg-slate-600 dark:text-white duration-300 transation-all ease-in-out":""}'}><div className="navbar ">
    ...
    <div className="navbarend space-x-3">
    <div className="navbar-center hidden lg:flex">
      <ul className="menu menu-horizontal px-1">{navbar}</ul>
    </div>
    <div className="hidden md:block">
    <label className="px-3 py-2 border rounded-md  flex items-center gap-2">
  <input type="text" className="grow outline-none dark:slate-1000 dark:text-white" placeholder="Search" />
  ...
  <input type="checkbox" className="theme-controller" checked={theme === "dark"} onChange={() => setTheme(theme === "light" ? "dark" : "light")} value="synthwave" />
